...
## 2026-09-24 — 命名复核的浏览器动作收敛

本地诊断显示已核验的会话在后续检查点反复进入命名元数据阶段，出现重复开观察页、刷新和关闭。行为回归用例先复现连续两次不必要的核验；修复后，通用 `document.title` 不触发复核，已由远端元数据证伪的同一标题差异不重复复核，新的具体标题变化仍触发核验。此前已核验的标题若在写前复核失败，保留上次核验记录并报告需显式 `organize`，避免后续 `wait` 自动循环。无标题写入时只使用首次新鲜元数据，省去第二次刷新。

`bun run check` 在收敛复核路径后通过 TypeScript 与 419 项测试；去除无写入时的额外刷新后，两个受影响测试文件的 40 项测试通过。隔离 Chrome for Testing 151.0.7922.34、agent-browser 0.34.0 的浏览器夹具通过，页面错误为空；该夹具使用 `data:text/html` 页面，没有应用 dev server 或 `APP_URL`，也不代表已在真实 ChatGPT 登录页验证风控结果。测试自有 Chrome、CDP 端口、controller daemon 和临时 profile 由脚本释放；共享登录 Chrome、隧道及持久任务数据未操作。

## 2026-09-25 — 已核验命名改用侧边栏标题
