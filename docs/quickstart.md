...
The caller submits the task objective, constraints and acceptance criteria through stdin or `--prompt`. `create` stores the original request, built-in guidance snapshot and complete rendered prompt in private `tasks.db` without opening a browser. Convorel adds the bound local workspace path, generic evidence/tool guidance and a `[CONVOREL:<runId>]` correlation marker. `start --id --run` and retries send that saved text unchanged, even after a runtime upgrade. Followups snapshot the current guidance for the new run. This is a normal ChatGPT user message, not a system-role message or an access grant. Source files are not embedded; the model retrieves relevant evidence through the advertised tools. Callers still supply task-specific references or required revisions. Review strategy belongs to the independently installable `chatgpt-review` skill.
