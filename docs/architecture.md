...
Tools: workspace_info, list_directory, tree, read_file, search_workspace, git_status, git_diff. Every tool declares an object output schema matching its structured response. workspace_info returns roots, mode and a nullable workspace detail object. tree reuses bounded directory listing and its disclosure policy, exposing depth/scan/page limits rather than claiming a complete dependency graph. No write_file or arbitrary shell tool. Fixed Git invocations disable external diff/textconv, fsmonitor, global config, lazy fetching and optional index updates; configured clean/process filters are refused; errors are failures, never a clean status.
