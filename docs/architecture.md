...
A local coding agent retains ownership of edits and tests. It talks to ChatGPT through the user's existing logged-in browser. ChatGPT can independently inspect the explicitly shared working directory through guarded MCP functions. This package provides the CLI, browser adapter, state, and MCP server. It does not include a model subscription, browser login, tunnel credentials, or hosted infrastructure.

Convorel owns the persistent conversation lifecycle and guarded local capabilities. It is the source of truth for task-to-conversation bindings, runs, delivery/completion state, saved replies and owned browser resources, and it keeps an additional content archive so archived prompts and replies stay searchable after the page is gone. The `conversation` API creates or reuses a conversation, continues it across turns and process restarts, reconciles interrupted operations, persists messages/results and releases owned tabs. Each new run snapshots the original task request, built-in workspace/tool guidance and the complete rendered prompt with a run correlation marker. The local workspace path comes from the task binding. Start, retry and rejected-send recovery use the stored rendered text, not a newly rendered template. Old runs are never retroactively rewritten. The guidance is part of a normal ChatGPT user message; it adds no system role, business persona or source contents. The bundled, independently installable `chatgpt-review` skill owns review triggers, context selection, reviewer instructions, findings and decision gates, including a bounded post-development comparison of the actual outcome against goals and architecture constraints. Result review does not default to a detailed code audit or full-log transfer. Generic tool/evidence guidance belongs to the runtime; review-specific questions and decision gates remain in the skill. The skill resolves a separately installed CLI through `PATH` as `convorel`, never relative to its own installation directory.
...
| Snapshot task input, built-in workspace/tool guidance and rendered prompt for each new run                        | Convorel                                                        |
...
创建时指定 `--type`/`--topic`（可选 `--language`），主题不明则保留原标题。配置项目时只从该项目专属输入框创建，并在填写和发送前核验项目 URL 与入口；没有先在外部创建再移动的路径。发送与观察阶段不改名；首次命名放到发送成功后的第一轮 wait 正常或超时返回前，前提是持久化 URL 已确认。后续 wait 返回和 finish 前只读核对当前会话 ID 对应的可见侧边栏标题；侧边栏条目缺失时不据此推断改名，已由远端元数据核验的同一标题差异不重复触发观察页，新的差异才进一步核验并纠正。此前已核验的标题若在新一轮复核时遇到写前读取错误，保留上次核验结果并停止自动重试，报告需显式 `organize`。生成中命名使用任务记录中的临时只读观察页获取远端元数据并核验保存，原发送页继续生成，观察页核验身份后关闭。命名失败单独保存，不改变投递状态、不自动重发，显式 `organize` 可在生成期间恢复命名。
...
- Task: user-supplied stable ID mapped to a workspace and conversation. Only an unsent initial prepared run permits explicit compare-and-rebind. For generated contexts, this updates the workspace in the rendered prompt and preserves the prior prompt/hash in the binding-change record; the original request and run ID remain unchanged. Historical prompts without generated context are preserved. Browser drafts are not rewritten.
...
- Run: UUID, original input/hash, versioned prompt context, final prompt/marker/hash, observed user message ID, observed model, reply and terminal outcome. Context is stored in the existing task document; no SQL schema migration is needed.
...
Lock ownership publication, release and dead-owner recovery share a dedicated SQLite writer mutex per lock name. Preferences use the same primitive around the full read/modify/write operation. These private mutex files contain no business data, remain on the same inode, and must not be deleted or replaced; the kernel releases their writer locks on process death. Callbacks are synchronous and short. Older processes that do not participate in this protocol must not share the state root concurrently. Owner metadata is published atomically, so interruption while staging it cannot publish a partial lock.
...
Archive publication rejects a run ID already owned by another task and rolls back the entire import. Selecting an existing content version again clears its superseded status and supersedes the previous selection. Its original `supersedes` creation edge remains immutable; this is content-version history, not a complete log of selection events.
...
Control actions prefer a scoped accessibility snapshot, an exact role/name match and a fresh element ref. The snapshot's global ref table is intersected with its scoped tree; ambiguous candidates never fall back to structural selectors. Refs are resolved after pacing and the final preflight, so a model-menu inspection cannot invalidate a previously selected Send ref. Structural selectors remain a fallback for controls without recognized accessible names. Send readiness independently checks the composer form, submit type, enabled state and obstruction; Stop and user/code Copy controls cannot prove completion. Conversation and message IDs remain separate from control names. A compact Pro trigger is verified through its full model menu; nested menu inspection reuses the adapter's pacing lease and settles before the outer Send dispatch.
...
The MCP surface is four functions: exec, memory, artifact and capabilities (`functions-v1`). Existing query handlers and output schemas remain internal operations, all sharing WorkspaceAccess. exec parses a bounded command string with shell-quote and strict argument schemas; it does not evaluate a host shell. Query results retain their original evidence contract inside execution.result. Each public function has an object output schema validated by the SDK, including nested discriminated result variants. capabilities exposes identity, operation schemas, runtime requirements and explicit grants.

Bun build/test scripts run through a separate Linux Bubblewrap executor over a bounded, policy-filtered source snapshot, with a clean environment and no host HOME, Git metadata or network. Only explicitly granted node_modules directories are shared read-only. Writable state lives in bounded temporary filesystems and is discarded; bounded execution reports are retained privately. Time/output/process/CPU/file limits and cancellation terminate the task without a host fallback. Memory has no total cgroup quota; system runtime files under /usr are readable. This execution scope is not a general remote terminal, nor proof that a test behaves identically in the host environment.

memory delegates search/read to the configured local ov CLI, enforcing explicit canonical URI subtrees before invocation and filtering search hits before returning a strict response. It never accepts executable paths, credentials, configuration flags or write/delete operations from callers. Unconfigured memory fails explicitly.

File discovery uses Bun's existing Glob matcher against the permission-filtered inventory. Literal content search accepts a directory scope and file pattern, returns context and hashes, and distinguishes pagination from scan/depth limits and unreadable files. Text reports use artifact(kind=text); artifact(kind=image) returns bounded native MCP PNG/JPEG/WebP content through the same policy. Reports and screenshots remain caller-provided evidence: their bytes and observation times do not certify execution, originating revision or test success. Private execution reports are not readable through artifact; callers receive the bounded execution response.
...
## Managed subprocess lifetime

Ordinary commands, memory backend calls and tunnel clients use a live session anchor. A private lifetime pipe reclaims the anchored process group if the caller dies; normal cleanup checks Linux boot/start identities, terminates owned session members, and bounds pipe draining. Tunnel records retain the anchor identity until cleanup succeeds, including client abnormal exit. This lifecycle mechanism is for cooperative subprocesses, not a security sandbox: a daemon deliberately creating a separate session has an independent lifetime. MCP build/test isolation remains in Bubblewrap.
