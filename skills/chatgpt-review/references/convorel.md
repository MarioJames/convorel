...
每次 `wait` 正常结束或到达观察超时后，在返回前被动读取当前页面标题并检查命名；`finish` 也先检查。已核验命名遇到通用页面标题或同一标题差异已被远端元数据证伪时，不重复开观察页；出现新的具体标题变化时才进一步核验并纠正。已核验标题的新一轮复核若在写前读取失败，返回 `revalidation_failed` 并停止自动重试，按回执显式 `organize`；首次命名前的临时故障仍可在后续检查点再试。不重发消息、不覆盖回复。这个命名步骤有界，但可能使命令晚于观察超时返回；SIGINT/SIGTERM 取消等待时跳过该步骤。权限/登录/身份冲突不会自动重试，编辑中断和不明保存仍按持久阶段处理。命名读回的元数据始终使用任务自有观察页，避免刷新原回复页。
